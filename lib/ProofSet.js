...
module.exports = class ProofSet {
  ...
  async verify(document, {
    ...
    console.log('verify -> legacy', legacy);
    if(legacy) {
    ...
    if(documentLoader) {
      ...
      console.log('verify -> strictDocumentLoader set');
      documentLoader = strictDocumentLoader;
    }
    if(expansionMap !== false) {
      expansionMap = strictExpansionMap;
    ...
    try {
      ...
      if(results.length === 0) {
        const suiteTypes =
          suites ? suites.map(s => s.type).join(',') : 'undefined';
        throw new Error(
          `Could not verify any proofs; no proofs matched the required
          suite(s) ${suiteTypes} and purpose ${purpose.term}
          for document ${doc.id}.`);
      }
...
async function _getProofs({
  ...
  if(proofSet.length === 0) {
    // no possible matches
    throw new Error(`No matching proofs found in the document ${document.id}.`);
  }
